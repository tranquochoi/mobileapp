        ...
        fun AddScreen(navController: NavController, firestoreRepository: FirestoreRepository) {
            ...
            LaunchedEffect(shouldReload) {
                if (shouldReload) {
                    ...
                    shouldReload = false // Reset shouldReload after reloading
                }
            }

            // Use LaunchedEffect to call suspend function only once when the screen is launched or data changed
            LaunchedEffect(Unit) {
                firestoreRepository.getAddCollection().let { notes ->
                    notesSnapshot = notes.reversed() // Reverse the list
                    checkedStates = MutableList(notes.size) { false } // Initialize checkbox state
                }
            }
            ...
            Scaffold(
                                ...
                                Text(
                                    ...
                                    modifier = Modifier
                                        .clickable {
                                            ...
                                            isDeleteVisible = false // Hide delete button
                                            showCheckboxes = false // Hide checkboxes
                                        }
                ...
                content = { innerPadding ->
                    Column(modifier = Modifier.padding(innerPadding)) {
                            ...
                            if (menuExpanded) {
                                DropdownMenu(
                                    ...
                                    DropdownMenuItem(onClick = {
                                        // Handle event when selected
                                        menuExpanded = false
                                        showCheckboxes = true // Show checkboxes when selected from menu
                                    }) {
                                        Text(text = "Chọn")
                                    }
                        ...
                        LazyColumn(
                            modifier = Modifier.padding(top = 8.dp) // Spacing between topAppBar and content below
                        ) {
                            ...
                            DeleteConfirmationDialog(
                                onDeleteConfirmed = {
                                    coroutineScope.launch {
                                        ...
                                        firestoreRepository.deleteNoteFromCollection("add", notesSnapshot.filterIndexed { index, _ -> checkedStates[index] })
                                        showDeleteConfirmation = false

                                        // Update note list after deletion
                                        notesSnapshot = firestoreRepository.getAddCollection().reversed()
